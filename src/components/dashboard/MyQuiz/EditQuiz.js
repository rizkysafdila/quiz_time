...
class EditQuiz extends Component {
  ...
  componentDidMount() {
    ...
    axios.get("http://127.0.0.1:9000/api/my-quiz/" + id + "/edit").then((res) => {
      this.setState({
        quiz_name: res.data[0].quiz_name,
  ...
  updateQuiz = () => {
    ...
    axios
      .put("http://127.0.0.1:9000/api/my-quiz/" + id + "/update", { quiz })
      .then((res) => {
        if (res.data.success) {
          this.setState({
            // questions: [],
  ...
  render() {
    return (
      <>
        <Toast model={this.state.showToast} message="Quiz Updated" />
          ...
          <div class="px-3 py-3 lg:px-5 lg:pl-3">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-1">
                ...
                <button type="button" onClick={() => this.updateQuiz()} class="focus:outline-none text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-900">
                  Update Quiz
                </button>
              </div>
            </div>
